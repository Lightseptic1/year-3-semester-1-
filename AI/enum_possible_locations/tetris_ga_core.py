...
def extract_features(board: Board, lines_cleared: int) -> List[float]:
    """
    Compute a feature vector from the board and the
...
def evaluate_board(board: Board, lines_cleared: int, genome: List[float]) -> float:
    ...
    Uses a neural network whose parameters are given by genome.
    """
    features = extract_features(board, lines_cleared)
    return nn_forward(features, genome)


# ========== MOVE GENERATION (ALL FINAL POSITIONS) ==========

def all_possible_moves(board: Board, shape_name: str):
    """
    Enumerate all possible final placements for a piece:
    for each rotation, for each x, drop until collision, lock, clear lines.
    """
    results = []
...
def evolve_one_generation(
    ...
    """
    Do one GA step: evaluate, sort, keep elites, breed/mutate rest.
    global_best_weights (if given) overwrites new_pop[0] so the best so far
    is always preserved.
    """
    ...
    stats: Dict[str, Any] = {
        "best_weights": best_weights,         # genome vector
...
# ========== TRAINING LOOP (PRINTS COMMA-SEPARATED BEST WEIGHTS) ==========
...
if __name__ == "__main__":
    ...
    global_best_genome: List[float] | None = None
    ...
    for gen in range(GENERATIONS):
        population, stats = evolve_one_generation(
            ...
            global_best_weights=global_best_genome,
        )

        best_fit = stats["best_fitness"]
        all_fit = stats["sorted_fitnesses"]
        gen_avg = sum(all_fit) / len(all_fit)

        # check for new global best
        new_global = False
        if best_fit > global_best_fitness:
            global_best_fitness = best_fit
            global_best_genome = stats["best_weights"].copy()
            new_global = True

            print("\n🔥 NEW GLOBAL BEST FOUND")
            print(f"Gen {gen+1}  Fitness: {global_best_fitness:.2f}")
            print("Weights (comma separated, paste into GUI):")
            print(",".join(f"{w:.6f}" for w in global_best_genome))

        print(f"\nGeneration {gen+1}")
        print(f"  Best fitness:    {best_fit:.2f}")
        print(f"  Average fitness: {gen_avg:.2f}")

        # kill generation if average dropped and no new best
        if prev_avg_fitness is not None and gen_avg < prev_avg_fitness and not new_global:
            print("  💀 Generation killed (avg dropped, no new global best). Resetting population...")
            # rebuild around global best + randoms
            if global_best_genome is None:
                population = [random_weights() for _ in range(POP_SIZE)]
            else:
                population = [global_best_genome.copy()]
                while len(population) < POP_SIZE:
                    population.append(random_weights())
            # do not update prev_avg_fitness from this bad gen
            continue

        prev_avg_fitness = gen_avg

    # final print
    print("\n✅ TRAINING COMPLETE")
    print(f"BEST EVER FITNESS: {global_best_fitness:.2f}")
    if global_best_genome is not None:
        print("FINAL WEIGHTS FOR GUI (comma separated):")
        print(",".join(f"{w:.6f}" for w in global_best_genome))
    else:
        print("No valid genome found. Something went very wrong.")
